...
class FindItem extends Component {
  state = {
    ...
    DisplayPicture: imagesList[0].imageUrl,
    count: 0,
    sec1: 60,
    isCheck: false,
  }
  ...
  playIcon = () => {
    const {sec1} = this.state
    if (sec1 > 0) {
      this.setState(prevState => ({
        sec1: prevState.sec1 - 1,
      }))
    }
    if (sec1 === 0) {
  ...
  shuffledEmojisList = imageUrl => {
    ...
    if (imageUrl === DisplayPicture) {
      ...
      this.setState({DisplayPicture: imagesList[RandomNumber].imageUrl})
      this.setState(prevState => ({count: prevState.count + 1}))
    } else {
      this.setState({isCheck: true, sec1: 0})
    }
  }
  ...
  getFunction = () => {
    ...
    const imagesList12 = imagesList.filter(each => each.category === ActiveId)
    return imagesList12
  }

  render() {
    const {ActiveId, DisplayPicture, count, sec1, isCheck} = this.state
    const DisplayItem = this.getFunction()
    return (
      <>
        <ul className="UnOrdered123">
          <li className="list">
            <NAVBAR Time={count} sec={sec1} />
          </li>
        </ul>

        {isCheck ? (
          <div className="FindItemContainer1">
            <div className="img12345">
              ...
              />
              <p className="heading123">YOUR SCORE</p>
              <h1 className="heading123">{count}</h1>
              <button
                type="button"
          ...
          <div className="FindItemContainer">
            <li className="list">
              <img src={DisplayPicture} alt="match" className="FirstImage" />
            </li>
            <ul className="unOrderList1">
              {tabsList.map(eachItem => (
                <TabItem
                  ...
                  isActive={ActiveId === eachItem.tabId}
                />
              ))}
            </ul>
            <ul className="unOrderList">
              {DisplayItem.map(each => (
                <Display
                  user={each}
                  key={each.id}
